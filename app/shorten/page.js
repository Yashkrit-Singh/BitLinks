...
const Shorten = () => {
  ...
  const generate = async () => {
    ...
    fetch("/api/generate", requestOptions)
      ...
      .then((result) => {
        setGenerated(`${process.env.HOST}/${shorturl}`);
        seturl("");
        setshorturl("");
        console.log(result);
    ...
    <div className="mx-auto max-w-lg bg-purple-100 my-16 p-8 rounded-lg flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        <input
          ...
          className="px-4 py-2 bg-white rounded-md focus:outline-purple-600"
          onChange={(e) => {
        ...
        <button
          onClick={generate}
          className="cursor-pointer bg-purple-500 rounded-lg shadow-lg p-3 py-1 my-3 font-bold text-white"
        >
          Generate
        </button>
      </div>
